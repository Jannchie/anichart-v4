import type { Data } from '@anichart/core'
...
// id / 国家码 → 图片 URL；loadImageMap 写入后再异步加载为纹理存入 core 的 textureMap
...
export function GoComposition() {
  ...
  useEffect(() => {
    init({
      ...
      bar.current.update(frameRef.current)
      topInfo.current.update(frameRef.current)
