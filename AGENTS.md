...
The US market-cap race (`stocks.csv`, playground/studio) is generated by
`scripts/update-stocks-data.py` from two keyless primary sources: Yahoo Finance
monthly closes (split-adjusted to *today's* basis) and SEC EDGAR XBRL shares
outstanding (reported in *then-current* units). The whole script exists to
reconcile those two bases: each share figure is scaled up by the cumulative
split factor occurring after its **filing date** (not its period-end — later
filings restate earlier periods in post-split units), so `close × adjusted
shares` is a consistent, split-invariant market cap. Shares fall back through
three XBRL concepts (cover-page → balance-sheet → weighted-avg-diluted) to cover
dual-class filers; isolated SEC glitches (e.g. a quarter reported 1000× off) are
median-filtered out of the per-company shares series. SEC's structured XBRL only
starts mid-2009, so for **2000–2009** the script scrapes share counts off the
cover page of each company's old 10-Ks (EDGAR full-text). Cover phrasing varies
wildly across filers, so rather than one brittle regex it harvests *every*
number near a "shares … outstanding" context as a candidate, then walks the
series newest→oldest picking the candidate closest to the running target
(seeded by the 2009 XBRL value) — authorised-share counts and dollar figures
fall outside the ±tolerance and are discarded. Monthly, **2000→present**.
Company display names double as `public/logos/` filenames, downloaded via
`--logos` (Google favicon service).

**SpaceX** is special-cased (`spacex_rows`): private before its June-2026 IPO, so
pre-IPO bars use reported funding-round / tender-offer valuations (a different,
clearly-labelled basis — no daily liquidity) and post-IPO bars use real market
cap (SPCX close × shares). Known gaps: Berkshire's 1500:1 A/B classes don't sum,
so it's omitted; a CIK change after a reorg (Alphabet, Disney) hides pre-reorg
history, and a few filers (Citigroup) use cover layouts the scraper can't parse —
all of these simply fade in when their data starts, as a race expects.
